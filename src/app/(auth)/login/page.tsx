...
export default function StudentLoginPage() {
  ...
  const handleGoogleSSO = () => {
    setError('Google SSO requires API keys to be configured in the backend. Please use email and password for now.');
  };
      ...
      <div className="w-full max-w-md bg-white rounded-3xl p-8 border border-slate-200/80 shadow-elevated space-y-6">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                autoComplete="email"
                required
                placeholder="e.g. rahim@skillhub.test"
                value={emailOrPhone}
                onChange={(e) => setEmailOrPhone(e.target.value)}
                className="w-full pl-10 pr-4 py-2.5 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-primary-500 text-slate-900"
              />
            </div>
          </div>
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full py-3 mt-4 rounded-xl bg-primary-600 hover:bg-primary-500 text-white font-semibold text-xs shadow-md shadow-primary-600/20 transition-all flex items-center justify-center cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
          >
